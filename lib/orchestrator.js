...
async function triggerManagerReview(managerAgent, reason) {
  ...
  // ✅ AJOUT : Check if agent is in discovery phase - skip auto-review
  ...
  if (activeTaskId) {
    ...
    if (taskPhase === 'discovery') {
      log(`[ORCHESTRATOR] Agent ${managerAgent.name} in discovery phase - skipping auto-review`);
      return; // ✅ Exit early, no review spawning during discovery
    }
  }
