...
This is the ONLY authoritative catalog. NEVER guess tool names or parameters. If a tool doesn't appear in this list, it doesn't exist.`;
  ...
  const armWatchdogIfReady = () => {
    ...
    // Always arm 10s after final result. Bg jobs are tracked via OS PID by
    // the bg-watcher (lib/bg-watcher.js), so they survive the CLI exit
    // independently. We no longer wait for `task_notification` here — that
    // signal is only useful while the CLI is alive, and tying the slot to
    // bg lifetime caused indefinite blocks (e.g. `php -S` dev servers).
    finalOutputWatchdog = setTimeout(() => {
      if (!processHandles.has(taskId)) return;
      log(`[TASK ${taskId}] FINAL_OUTPUT grace expired — terminating CLI (bg=${activeBgTasks.size})`);
      activityStream.write(`[${logTimestamp()}] FINAL_OUTPUT grace expired — terminating (bg=${activeBgTasks.size})\n`);
  ...
  const parseCallbacks = {
    ...
    onBgTaskStarted(event) {
      ...
      // We do NOT disarm the watchdog here: the watchdog is independent of
      // bg tasks now (see armWatchdogIfReady). Bg jobs are tracked by the
      // bg-watcher via OS PID, surviving the CLI exit.
    ...
    onBgTaskNotification(event) {
      ...
      // Bridge is centralised in lib/bg-watcher.js (OS-level PID polling
      // detects completion even after the CLI exits, so the bridge survives
      // CLI shutdown and works for any future runner profile too).
      // Note: we no longer call armWatchdogIfReady() here — the watchdog is
      // bg-independent now (10s after FINAL_OUTPUT, period).
    },
  };
