...
/**
 * Non-destructive existence check on a host-OS PID. Mirrors the helper in
 * lib/bg-watcher.js so the spawner can decide whether to honour a CLI
 * task_notification:completed or defer to the bg-watcher.
 */
...
This is the ONLY authoritative catalog. NEVER guess tool names or parameters. If a tool doesn't appear in this list, it doesn't exist.`;
  ...
  // block-* hooks no-op for Yabby super agent (managed elsewhere), but
  // bg-pid-capture applies to every claude agent — the bg-watcher needs the
  // PID file to detect completion of any bg job.
  let settingsPath = null;
  if (runnerId === "claude" && agentId) {
    try {
      const isYabbySuper = agentId === "yabby-000000";
      const preToolUse = [];
      ...
      if (!isYabbySuper) {
        ...
        // Lead-coding gate: only attached when the agent is a lead.
        if (agentIsLead) {
      ...
      // bg-pid-capture wraps Bash(run_in_background=true) with
      //   `sh -c 'echo $$ > /tmp/yabby-bg/<tool_use_id>.pid; exec <orig>'`
      // so the bg-watcher can detect completion via `kill -0 <pid>`.
      // Applied to every agent (Yabby included) so the user gets a
      // [BG_COMPLETED] follow-up when a long-running bg job ends.
      const bgPidHookScript = join(process.cwd(), "scripts/claude-hook-bg-pid-capture.js");
      preToolUse.push({
        matcher: "Bash",
        hooks: [{ type: "command", command: `node ${bgPidHookScript}`, timeout: 5 }],
      });
  ...
  const parseCallbacks = {
    ...
    onBgTaskStarted(event) {
      ...
      const exitFile = toolUseId ? `/tmp/yabby-bg/${toolUseId}.exit` : null;
      const serviceMarker = toolUseId ? `/tmp/yabby-bg/${toolUseId}.service` : null;
      const isService = serviceMarker ? existsSync(serviceMarker) : false;
    ...
    onBgTaskNotification(event) {
      ...
      // The CLI's task_notification fires as soon as the tool call returns
      // — for shell-internal `&` (e.g. `python3 script.py &`) that's a few
      // ms, even though the actual process runs for minutes. If we have a
      // tracked PID and it's still alive, defer to the bg-watcher: keep
      // the row 'running' so it can detect real completion via `kill -0`.
      trackBgWrite(taskId, (async () => {
        try {
          const { getBgTaskByCliId, markBgTaskNotification } = await import("../db/queries/bg-tasks.js");
          if (status === "completed") {
            const row = await getBgTaskByCliId(cliTaskId);
            if (row && row.pid && isPidAlive(row.pid)) {
              log(`[TASK ${taskId}] BG_NOTIF: ${cliTaskId} — CLI says completed but pid ${row.pid} still alive, deferring to bg-watcher`);
              activityStream.write(`[${logTimestamp()}] BG_NOTIF: ${cliTaskId} deferred (pid ${row.pid} alive)\n`);
              return; // keep activeBgTasks set; watcher will resolve later
            }
          }
