...
This is the ONLY authoritative catalog. NEVER guess tool names or parameters. If a tool doesn't appear in this list, it doesn't exist.`;
  ...
  // FINAL_OUTPUT watchdog: armed when the CLI emits its final result. After
  // 10s of grace, SIGTERM the process group if the CLI hasn't exited on its
  // own. killedAfterFinalOutput tells the close handler to treat the forced
  // exit as a successful completion (we already captured the result).
  ...
  const parseCallbacks = {
    ...
    onResult(costUsd, durationMs, resultData) {
      ...
      // FINAL_OUTPUT watchdog: the CLI just produced its final result. Give it
      // 10s to exit naturally; if not, SIGTERM the whole process group. The
      // existing close handler will then persist resultSummary and emit
      // status=done — no duplicate writes, no new events.
      if (!finalOutputWatchdog) {
        finalOutputWatchdog = setTimeout(() => {
          if (!processHandles.has(taskId)) return; // already exited
  ...
  child.on("close", async (code) => {
    ...
    // Clear the watchdog timer (covers both natural exit and watchdog-driven exit).
    if (finalOutputWatchdog) { clearTimeout(finalOutputWatchdog); finalOutputWatchdog = null; }

    // Reap any orphaned children the CLI left behind (zsh + curl sub-process
    // spawned via Bash tool calls don't always get cleaned up — they end up
    // blocked on dead sockets after the CLI itself exits). SIGKILL the whole
    // group; the CLI is already gone so this only hits leftover descendants.
